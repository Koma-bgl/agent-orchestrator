...
export function saveDraft(d = {}) {
	const clean = {};
	for (const k of ["owner", "repo", "teamId", "label", "statusName"])
		if (d[k] != null && d[k] !== "") clean[k] = String(d[k]);
...
export async function githubDeviceStart() {
	if (!GH_CLIENT_ID) {
		const e = new Error(
			"GITHUB_OAUTH_CLIENT_ID is not set on this bot. Register a GitHub OAuth App (device flow enabled) and set its client id.",
		);
		e.code = 400;
		throw e;
	}
	...
	if (!j.device_code) {
		const e = new Error(`GitHub device/code failed: ${j.error_description || j.error || `HTTP ${r.status}`}`);
		e.code = 502;
		throw e;
	}
...
export async function githubDevicePoll() {
	if (!GH_CLIENT_ID) {
		const e = new Error("GITHUB_OAUTH_CLIENT_ID is not set.");
		e.code = 400;
		throw e;
	}
	if (!deviceFlow) {
		const e = new Error("no device-login in progress — start one first");
		e.code = 400;
		throw e;
	}
...
export function shellStart({ which = "github" } = {}) {
	...
	const proc = spawn(
		...
		[
			...
			cfg.session,
			"bash",
			"/app/scripts/" + cfg.script,
		],
		{ detached: true, stdio: "ignore", env: process.env },
	);
	proc.on("error", () => {
		shellProcs[which] = null;
	});
	proc.on("exit", () => {
		shellProcs[which] = null;
	});
...
export async function githubConnect({ pat }) {
	if (!pat || !pat.trim()) {
		const e = new Error("pat required");
		e.code = 400;
		throw e;
	}
	...
	const st = await githubStatus();
	if (!st.connected) {
		const e = new Error("gh auth did not persist");
		e.code = 502;
		throw e;
	}
	return st;
...
export async function githubOwners() {
	...
	try {
		...
		/* no orgs / no read:org — just personal */
	}
...
export async function githubRepos({ search, owner } = {}) {
	...
	try {
		...
		if (/SAML|SSO|401|403/i.test(msg)) {
			const err = new Error(
				`GitHub 403 — your login likely needs SSO authorization for ${owner || "this org"} (authorize it in the org's settings, then retry).`,
			);
			err.code = 403;
			throw err;
		}
		const err = new Error(`gh repo list failed: ${msg}`);
		err.code = 502;
		throw err;
	}
}
...
export function writeTokens({ linear, anthropic, claude } = {}) {
	...
	const clean = (v) => {
		...
		if (val && /[\r\n]/.test(val)) {
			const e = new Error("token must not contain newlines");
			e.code = 400;
			throw e;
		}
		return val;
	...
	if (tok) {
		if (tok.startsWith("sk-ant-oat")) {
			...
			const e = new Error(
				"Claude token must start with sk-ant-oat… (from `claude setup-token`) or sk-ant-api… (API key). That doesn't look like either — re-copy the full token.",
			);
			e.code = 400;
			throw e;
		}
...
async function linearQuery(key, query, variables) {
	...
	if (!r.ok) {
		const e = new Error(`Linear API HTTP ${r.status} (key invalid?)`);
		e.code = 400;
		throw e;
	}
	const j = await r.json();
	if (j.errors) {
		const e = new Error("Linear API error (key invalid?)");
		e.code = 400;
		throw e;
	}
	return j.data;
...
export async function linearTeams({ apiKey } = {}) {
	...
	if (!key) {
		const e = new Error("no LINEAR_API_KEY (save it first or pass apiKey)");
		e.code = 400;
		throw e;
	}
...
export async function linearLabels({ apiKey, teamId } = {}) {
	...
	try {
		...
		const data = teamId
			? await linearQuery(
					key,
					"query($f: IssueLabelFilter) { issueLabels(first: 250, filter: $f) { nodes { name } } }",
					{ f: { team: { id: { eq: teamId } } } },
				)
...
export async function linearStatuses({ apiKey, teamId } = {}) {
	...
	try {
		const data = teamId
			? await linearQuery(
					key,
					"query($f: WorkflowStateFilter) { workflowStates(first: 100, filter: $f) { nodes { name type position } } }",
					{ f: { team: { id: { eq: teamId } } } },
				)
			: await linearQuery(key, ALL);
	...
	for (const n of nodes) {
		if (!seen.has(n.name)) {
			seen.add(n.name);
			statuses.push({ name: n.name, type: n.type });
		}
	}
	return { statuses };
}
...
export async function applyProject({ repo, teamId, label, labels, statusName } = {}) {
	if (!repo || !teamId) {
		const e = new Error("repo and teamId required");
		e.code = 400;
		throw e;
	}
