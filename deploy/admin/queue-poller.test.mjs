...
test("formatPrTitle: conventional feat with scope -> [Feat][TICKET]", () => {
	assert.equal(formatPrTitle("feat(sports): add cashout card", "SPOR-1"), "[Feat][SPOR-1] Add cashout card");
});
...
test("classifyPane: a live claude session with an in-flight turn is busy", () => {
	assert.equal(classifyPane("⏵⏵ bypass permissions on (shift+tab to cycle) · esc to interrupt · ← for age…"), "busy");
});
...
test("classifyPane: busy wins over shell — never touch a pane mid-turn", () => {
	assert.equal(classifyPane("root@host:~/w# something\nesc to interrupt"), "busy");
});
