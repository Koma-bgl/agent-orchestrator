...
function httpCode(url) {
	try {
		return run("curl", ["-sk", "-o", "/dev/null", "-w", "%{http_code}", url]).trim();
	} catch (e) {
		return (e.stdout || "000").toString().trim();
	}
}
...
function preflight() {
	...
	if (!hasToken) {
		console.error("✗ no GCP credential (set AO_GCP_ACCESS_TOKEN, run `gcloud auth login`, or ADC)");
		process.exit(1);
	}
	console.log("✓ GCP credential available");
	const project = resolveProject();
	if (!project) {
		console.error("✗ no GCP project (pass --project= or `gcloud config set project`)");
		process.exit(1);
	}
...
const CREATE_HINTS = {
	"google-oauth-client":
		"Console: OAuth Web client, redirect https://localhost:8443/auth/oauth2/google/authorization-code-callback; then: printf '%s' 'ID|SECRET' | gcloud secrets create google-oauth-client --data-file=-",
...
function check() {
	...
	for (const s of GATE_SECRETS) {
		...
		if (!exists) {
			missing++;
			console.log(`      create: gcloud … --project=${project}  →  ${CREATE_HINTS[s]}`);
		}
	}
	if (missing) {
		console.error(`\n${missing} gate secret(s) missing — create them, then re-run.`);
		process.exit(1);
	}
...
function up() {
	...
	const env = buildEnv({
		...
		jwt,
		allowlist,
		watchtowerToken: randomBytes(24).toString("hex"),
	});
	writeFileSync(join(DEPLOY_DIR, ".env"), env);
	console.log(
		`Wrote deploy/.env (client_id ${id.length}c, secret ${secret.length}c, jwt ${jwt.length}c, allowlist ${allowlist.length}c) — gitignored.`,
	);
...
function verify() {
	...
	checks.push([
		"daemon /healthz (in-container)",
		ok("docker", [...COMPOSE, "exec", "-T", "ao", "curl", "-fsS", "http://127.0.0.1:3001/healthz"]),
	]);
	let realOauth = false;
	try {
		const loc =
			run("curl", ["-sk", "-i", `${SITE}/auth/oauth2/google`])
				.split("\n")
...
if (!table[cmd]) {
	console.error("usage: run.mjs preflight|check|up|verify|down [--project=ID] [--wipe]");
	process.exit(2);
}
table[cmd]();
