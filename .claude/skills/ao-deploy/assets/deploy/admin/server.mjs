...
async function getVersion() {
	let running = "unknown";
	try {
		const { stdout } = await execFileAsync("ao", ["--version"], { timeout: 10000 });
		running = stdout.trim();
	} catch {}
	let latest = null;
	try {
...
async function rotateSecret(body) {
	...
	if (!isValidSecret(secret)) {
		const e = new Error(`unknown secret (valid: ${secretNames().join(", ")})`);
		e.code = 400;
		throw e;
	}
	if (chooseSource(process.env) !== "gcp") {
		const e = new Error("rotation requires the gcp secret source (AO_GCP_PROJECT)");
		e.code = 400;
		throw e;
	}
	...
	if (!r.ok) {
		const e = new Error(`Secret Manager addVersion failed: HTTP ${r.status}`);
		e.code = 502;
		throw e;
	}
...
createServer(async (req, res) => {
	try {
		...
		if (POST && pathname === "/admin/api/github/connect")
			return send(res, 200, await githubConnect(await readBody(req)));
		...
		if (GET && pathname === "/admin/api/github/repos")
			return send(
				res,
				200,
				await githubRepos({
		...
		if (POST && pathname === "/admin/api/linear/statuses")
			return send(res, 200, await linearStatuses(await readBody(req)));
		...
		if (GET && pathname === "/") {
			...
			try {
				configured = !!getFirstProject(CONFIG_PATH);
			} catch {
